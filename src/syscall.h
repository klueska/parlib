...
#define async_malloc(...) async_call_start(malloc, __VA_ARGS__)
#define async_malloc_detached(callback, ...) \
  async_call_start_detached(void *, callback, malloc, __VA_ARGS__)

#define open(...) __async_simulate_blocking(int, open, __VA_ARGS__)
#define async_open(...) async_call_start(open, __VA_ARGS__)
#define async_open_detached(callback, ...) \
  async_call_start_detached(int, callback, open, __VA_ARGS__)

#define close(...) __async_simulate_blocking(int, close, __VA_ARGS__)
#define async_close(...) async_call_start(close, __VA_ARGS__)
#define async_close_detached(callback, ...) \
  async_call_start_detached(int, callback, close, __VA_ARGS__)

#define unlink(...) __async_simulate_blocking(int, unlink, __VA_ARGS__)
#define async_unlink(...) async_call_start(unlink, __VA_ARGS__)
#define async_unlink_detached(callback, ...) \
  async_call_start_detached(int, callback, unlink, __VA_ARGS__)
...
({ \
  void *result; \
  void *handle = async_call_start(__func, __VA_ARGS__); \
  async_call_wait(handle, &result); \
  (return_type)result; \
})

#define async_call_start(__func, ...) \
({ \
  void *do_##__func(void *arg) { \
    return (void*)(long)__safe_call(__func, __VA_ARGS__); \
  } \
  __async_call_start((void*)(&(do_##__func)), false); \
})

#define async_call_start_detached(return_type, callback, __func, ...) \
({ \
  void *do_##__func(void *arg) { \
  ...
  __async_call_start((void*)do_##__func, true); \
})

#define async_call_wait(handle, result) \
  __async_call_wait(handle, ((void**)result));
